...
- **Semantic Runtime Patrol**: patrol AI juga membaca `capital_governor.json`, `live_order_dispatcher.json`, top target boards, scanner status, dan Telegram throttle/API. Kondisi seperti target ada tetapi order OFF, hard stop aktif, rollover pending, atau Telegram tidak siap menjadi status eksplisit, bukan sekadar “service active”.
- **Workflow Automation Supervisor**: `Core/Support/workflow_supervisor.py` dan `kibot-workflow-supervisor.service` merangkum alur uang -> scanner -> target board -> risk governor -> dispatcher -> Telegram -> AI support ke `state/workflow_automation.json`, sehingga sistem punya satu kontrak jelas untuk menjawab “kenapa trading jalan/diam dan langkah berikutnya apa”. Blocker yang bisa diperbaiki otomatis, seperti `daily_rollover_exit_pending`, diproses repair-first melalui daily reset/executor path; Telegram hanya eskalasi jika repair gagal atau blocker tetap persistent.
