...
- `kibot_ai_scout.py` menjaga scouting intel pasar dan berita global tetap hidup, sekaligus menjalankan runtime patrol 5 menit untuk service/log/tooling readiness termasuk GitHub CLI, Copilot, Aider, dan Crush.
...
- `trade_history.py` menulis riwayat perdagangan yang ringkas dan manusiawi ke `state/trade_history/YYYY-MM-DD.jsonl`, lalu memirror event itu ke decision journal supaya buy/exit/PnL bisa ditelusuri dari entry sampai close. Event close sekarang membawa `fee_idr`, `gross_realized_pnl_idr`, dan `net_realized_pnl_idr`, sehingga dashboard dan audit trail tidak lagi menganggap gross PnL sebagai profit bersih. Pending buy yang keburu lari dari pasar juga sekarang bisa dicatat sebagai stale/cancelled alih-alih diam selamanya di harga lama.
...
- Daily PnL has a reconciliation contract: `daily_equity_anchor.json` is the canonical start-of-day baseline, `capital_governor.json` supplies current equity and order permission, and `pnl_reconciliation.json` records mismatches plus what-if checks. When these disagree, new entries must stay blocked and the dashboard must expose the mismatch.
- Trade lifecycle logging now keeps pending entry states visible as `BUY PENDING` / `SELL PENDING` until the wallet delta settles, and the dashboard activity feed intentionally suppresses generic system spam so the operator sees trade events instead of repeated runtime noise.
