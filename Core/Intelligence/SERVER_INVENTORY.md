...
21. A visual control-plane dashboard now exists at `Core/Intelligence/kibot_dashboard.py` and is launched with `bin/kibot-dashboard` on port `8787`, so the whole delegation flow can be inspected in a browser. Dashboard V2 now serves an isometric agent canvas, live Indodax/Polymarket ledger, council lens, and event stream from `Core/Intelligence/dashboard/`.
