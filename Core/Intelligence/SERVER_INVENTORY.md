...
18. `EXIT_ALL` is clamped so it only applies in the midnight window or a genuine emergency; CPU-only spikes are downgraded to DEGRADED instead of a full trading shutdown when RAM/disk are still healthy. A healthy FLAT day is no longer allowed to default the council into `DEFENSIVE`; it is softened toward `CONTROLLED_AGGRESSIVE` or `NEUTRAL` so the system keeps seeking edges instead of going passive.
