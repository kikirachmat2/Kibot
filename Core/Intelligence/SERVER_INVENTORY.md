...
- `bin/kibotctl` operator wrapper for status / doctor / restart / sync-models and the expanded runtime service inventory.
...
14. `SovereignCouncil` now emits explicit `ENTER / WAIT / EXIT` posture and a recovery mode when equity is red but the evidence is still strong enough before midnight. Once the global daily loss cap is breached, the governor flips to `BLOCKED_WITH_REASON`, brains switch to `EXIT_ONLY`, and new entries are blocked by dispatcher/RiskGate even if one venue is otherwise healthy.
...
34. Pending buy entries are now tracked explicitly as `BUY PENDING` until the wallet delta resolves. The live dispatcher also treats open `OrderTracker` records as active symbols so the same pair cannot be re-entered while a buy is still in flight. When a buy limit drifts too far from market or ages out, the executor now cancels it instead of sitting on the old price forever.
35. Council trading deliberation is bounded. Web evidence calls and AI roles have explicit timeouts; if provider/search stalls, a deterministic local scorer uses confidence, spread, tick-size, price levels, OHLC quality, what-if edge, and deadline pressure to produce a `WAIT` or `COUNCIL_MANDATE`.
36. The deterministic fallback is restricted to tradeable Indodax IDR pairs for real execution. Universal lead-lag items such as exchange names can inform context, but cannot become buy mandates.
37. A manual Telegram status report was verified through `SovereignNotifier.send_status_reply()` with the shared throttle helper; it returned `TELEGRAM_STATUS_SENT True`.
38. `TRADING_STRATEGY.md` is now the canonical trading-intelligence contract. It covers Indodax pump lifecycle, green-builder fallback, Polymarket event trading, cross-exchange capital commander, deadline discipline, role-agent debate, online evidence, scanner/executor gaps, Telegram report template, and dashboard requirements.
39. `decision_journal.py` writes a daily JSONL audit trail under `state/decision_journal/`. Scanner candidates, council decisions, pre-trade simulations, executor events, and trade mirror events can now be traced after the fact.
40. `trade_history.py` keeps a concise buy/open/close/PnL journal under `state/trade_history/` and mirrors each event into `decision_journal`, so the operator can reconstruct trading history without scraping raw executor logs.
41. `market_heatmap.py` snapshots Indodax market breadth from live tickers and persists `state/market_heatmap.json`, giving Council a market-regime input instead of relying on isolated pair signals.
42. `probability_engine.py` persists `state/green_probability.json` from daily PnL color, deadline mode, market breadth, scanner slate, order quality, system health, and source health.
43. `pre_trade_simulator.py` runs before buy-side execution. It blocks entries with empty depth, excessive spread/slippage, unsellable minimum amount, or partial-TP infeasibility, and can recommend smaller size before RiskGate.
44. Indodax executor now stores `exit_plan`, `pre_trade_simulation`, `trade_grade`, `lifecycle`, `deadline_mode`, and `capital_state` inside `active_trades.json`, so every position carries its own reason, risk, and exit contract.
45. Exit management now supports partial take-profit, trailing stop, max-hold timeout, and distribution exits while still falling back to legacy hard stop / take profit for old positions.
46. Midnight Telegram reporting is routed through `SovereignNotifier.send_daily_report()` and `daily_report.py` with shared throttling, so the operator receives one compact report instead of spam.
47. Dashboard V3.1 now surfaces strategy intelligence: deadline mode, risk mode, quality floor, green probability, scanner slate, decision journal count, and market breadth in the visual workflow board.
48. Universal lead-lag signals are now context-only when they appear alone. They are persisted in scanner state, but Council is only awakened when there is at least one tradeable Indodax or Polymarket signal, preventing non-executable exchange names from consuming AI deliberation time.
49. Fallback category intelligence is implemented in `Core/Intelligence/coin_category.py`. Scanner now labels each Indodax candidate with deterministic category policy (`HIGH_LIQUIDITY_MAJOR`, `BTC_ETH_BETA`, `AI_BIG_DATA`, `RWA_DEFI`, `MEME_ROTATION`, `LOCAL_MOMENTUM`) so Council can switch from pump riding to structured green-builder mode without buying random dead coins.
50. The Indodax unit-price law is enforced by both `RiskGate` and `IndodaxExecutor`: a BUY is rejected when `price_idr >= total_equity_idr`. This makes the operator rule explicit: KiBot may only buy coins whose one-unit price is below the current total balance/equity.
51. Strategy documentation is now split by responsibility: `TRADING_STRATEGY.md` for trading, `SYSTEM_STRATEGY.md` for non-trading autonomy, `strategy/specs/AUTONOMY_GAP_REGISTER.md` for gap tracking, `strategy/roadmaps/IMPLEMENTATION_ROADMAP.md` for phased execution, `strategy/specs/INVENTORY_UTILIZATION.md` for server asset usage, `strategy/specs/SYSTEM_COMMANDER_SPEC.md` for the missing non-trading brain, `strategy/roadmaps/POLYMARKET_RUNTIME_ROADMAP.md` for Polymarket V2, and `strategy/specs/OBSERVABILITY_DASHBOARD_SPEC.md` for dashboard/control-plane visibility.
52. `SystemCommander` is now the canonical non-trading system brain. It writes `state/system_commander.json` and `state/inventory_matrix.json`, classifies health (`HEALTHY/DEGRADED/BLIND/UNSAFE`), scores canonical services/models/tools/state, reads provider/source health, exposes GitHub/server drift, and mirrors runtime snapshots into the decision journal so toolchain health is historized. It no longer treats absent Tailscale as a missing runtime dependency.
53. `scripts/build_ai_system_inventory.py` writes `state/ai_system_inventory.json` to provide a compact runtime inventory of AI, council, advisor, and orchestration components together with their live-only / lock status and evidence paths.
53. Dashboard System Brain now consumes a stable `system_brain` summary contract, so inventory utilization, source health, and drift status are rendered from live state instead of mismatched frontend IDs.
54. Backup automation now defaults to non-secret protected archives (`state/`, strategy docs, inventory) with private file permissions. Plaintext `.env` is excluded unless `KIBOT_BACKUP_INCLUDE_SECRETS=1` is deliberately set.
55. Server-only legacy systemd restart loops and stale timers were retired/disabled so missing historical `Batam/...` paths no longer spam logs or burn CPU. Canonical runtime remains `kibot-master`, `kibot-scanner`, `kibot-executor`, `kibot-executor-polymarket`, `kibot-ai-scout`, `kibot-janitor`, `kibot-dashboard`, `ollama`, and `redis-server`.
56. Solana trending meme hunter now exists as a guarded live-control module: `Core/Web3/solana_trending_scanner.py` feeds `state/solana_trending_candidates.json`, `Core/Web3/pumpfun_route_detector.py` / `Core/Web3/pumpfun_scanner.py` classify Pump.fun-native vs Jupiter-routable routes, `Core/Intelligence/strategy/solana_momentum_meme_strategy.py` scores small-cap momentum, and dashboard/control-plane surfaces the best candidate plus reject reason without enabling reserve spend or bypassing exit plans.
57. Autonomous sizing is now a runtime source-of-truth in `Core/Trading/autonomous_sizing.py`; executors consume its `size_idr` output instead of hardcoded fixed trade caps, and the latest sizing decision is written to `state/autonomous_sizing.json` and surfaced on the dashboard.
58. Live autonomous execution now has an explicit dispatcher service: `Core/Decision/live_order_dispatcher.py` signs HMAC `COUNCIL_MANDATE` payloads into the Indodax UDP executor from `state/indodax_top_targets.json`, rotates through executable targets with cooldowns, and records `state/live_order_dispatcher.json`. The same runtime state tracks Phantom same-chain dispatch attempts and exact blockers. Strategy snapshots are sanitized at load time so stale defensive pair locks cannot freeze Indodax during live autonomous mode.
59. Indodax exits are now fee-aware end to end: `Core/Intelligence/exit_plan.py` clamps partial TP / trailing thresholds to the minimum profitable exit above round-trip fees, `IndodaxExecutor` records fee-aware gross/net realized PnL on close, `OrderTracker.reconcile()` persists the reconciled PnL to disk before transition, and `trade_history.py` mirrors the net close with fee metadata so dashboard green/red reflects realized profit after costs instead of gross ticket value.
60. Web3 route selection now carries explicit `fee_intelligence`: `Core/Web3/web3_fee_intelligence.py` profiles Solana base fee + priority fee + gasless cap, Base L2 execution + L1 security fee, and bridge source/destination costs; `web3_quote_router.py`, `base_quote_router.py`, `web3_safety_checker.py`, `web3_executor_guard.py`, `phantom_router.py`, `bridge_router.py`, `pumpfun_scanner.py`, `solana_trending_scanner.py`, `pumpfun_live_runner.py`, `web3_exit_daemon.py`, and the dashboard/control plane all consume the same gas-aware contract before recommending entry or exit.
...
64. AI patrol now performs semantic runtime checks, not only liveness checks. It reads `capital_governor.json`, `live_order_dispatcher.json`, Indodax/Phantom top target boards, scanner source status, and Telegram throttle/API state; it records explicit runtime status when targets exist but orders are blocked, daily rollover is pending, global hard stop is active, dispatcher is blocked, Telegram is not configured/healthy, or scanner state contradicts fetched data. Auto-repairable runtime blockers are not sent as immediate Telegram alerts.
65. Workflow automation now has a dedicated supervisor: `Core/Support/workflow_supervisor.py`, `config/systemd/kibot-workflow-supervisor.service`, and `scripts/assert_workflow_automation.py`. It writes `state/workflow_automation.json` with service health, money truth, target counts, risk-governor status, dispatcher reason, Telegram health, support tools, blockers, and the next action. This is the canonical “why is the machine trading or not trading?” state. For `daily_rollover_exit_pending`, the supervisor triggers the daily reset/executor repair path first and only escalates through Telegram after persistent repair failure or non-repairable blockers.
66. Autonomy readiness now has dedicated audit writers and one-shot assertions: `scripts/audit_target_freshness.py`, `scripts/audit_ai_actual_usage.py`, `scripts/audit_autonomous_runtime_readiness.py`, `scripts/audit_server_extensions_usage.py`, `scripts/audit_repo_safety.py`, `scripts/audit_trading_decision_policy.py`, and `scripts/assert_boleh_ditinggal.py`. These generate state files for target freshness, real AI usage, runtime readiness, server extension usage, repo safety, and trading policy, so operators can answer “can I leave the system alone?” from canonical runtime evidence instead of guesswork.
