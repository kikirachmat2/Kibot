...
34. Pending buy entries are now tracked explicitly as `BUY PENDING` until the wallet delta resolves. The live dispatcher also treats open `OrderTracker` records as active symbols so the same pair cannot be re-entered while a buy is still in flight.
