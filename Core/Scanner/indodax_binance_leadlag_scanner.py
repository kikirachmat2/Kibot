...
class IndodaxBinanceLeadLagScanner:
    ...
    def __init__(self) -> None:
        ...
        self.lookback_sec = int(os.getenv("KIBOT_INDO_BINANCE_LEADLAG_LOOKBACK_SEC", "12"))
        self.min_leader_move_pct = float(os.getenv("KIBOT_INDO_BINANCE_LEADLAG_MIN_LEADER_MOVE_PCT", "0.45"))
        self.max_follower_move_pct = float(os.getenv("KIBOT_INDO_BINANCE_LEADLAG_MAX_FOLLOWER_MOVE_PCT", "0.25"))
        self.min_gap_pct = float(os.getenv("KIBOT_INDO_BINANCE_LEADLAG_MIN_GAP_PCT", "0.25"))
        self.min_volume_idr = float(os.getenv("KIBOT_INDO_BINANCE_LEADLAG_MIN_VOLUME_IDR", "50000000"))
        self.min_leader_quote_volume = float(os.getenv("KIBOT_INDO_BINANCE_LEADLAG_MIN_LEADER_QUOTE_VOL", "1500000"))
        self.max_spread_pct = float(os.getenv("KIBOT_INDO_BINANCE_LEADLAG_MAX_SPREAD_PCT", "0.80"))
        self.fee_roundtrip_pct = float(os.getenv("KIBOT_INDO_BINANCE_LEADLAG_FEE_ROUNDTRIP_PCT", "0.51"))
    ...
    def _build_candidate(
        ...
        expected_ok = expected_net_pct > 0.0
        ...
        if not leader_move_ok or not spread_ok or not volume_ok or not gap_ok or not expected_ok:
            action = "REJECT"
            route_status = "BLOCKED_WITH_REASON"
        elif gap_ok and follower_still_lagging and expected_ok:
            action = "ENTER"
