...
Updated: 2026-06-06 WIB / 2026-06-05 UTC.
...
| :--- | :--- | :--- |
| Core truth/control | `kibot-live-truth`, `kibot-capital-governor`, `kibot-scanner`, `kibot-target-board`, `kibot-autonomous-brain`, `kibot-indodax-director`, `kibot-live-dispatcher`, `kibot-executor`, `kibot-dashboard` | Keep active. These form the Indodax-only money truth, scanner, decision, dispatch, execution, and UI path. |
...
| Retired route runners | `kibot-phantom-brain`, `kibot-base`, `kibot-pumpfun`, `kibot-future-web3`, `kibot-executor-polymarket`, `kibot-web3-exit` | Disable and stop. Operator removed Phantom/Web3 after wallet compromise concern; runtime must be Indodax-only. |
...
* Retired Phantom/Web3 runtime from the canonical service graph. Total equity and PnL now exclude Phantom/Web3 state and use Indodax-only money truth.
...
Indodax-only contract audit command:

```bash
PYTHONPATH=. python scripts/assert_indodax_only_runtime.py
```
...
*   **Runtime Patrol:** `Core/Intelligence/kibot_ai_scout.py` writes `state/ai_patrol.json` every 5 minutes to verify service health, log anomalies, and AI/Copilot/GitHub/Crush tooling readiness without blocking live trading.
...
`Core/Intelligence/kibot_ai_scout.py` now verifies runtime truth beyond green `systemd` state. The patrol reads capital governor order permission, live dispatcher status, top target boards, scanner source status, and Telegram throttle/API readiness. If targets are visible but orders are blocked, daily rollover is pending, global hard stop is active, dispatcher is blocked, Telegram is unhealthy, or scanner state contradicts real fetched data, `state/ai_patrol.json` records explicit runtime status and `scripts/assert_runtime_semantics.py` can fail the operator check with the exact blocker. Auto-repairable rollover blockers are state-visible but not urgent Telegram spam.

`Core/Support/workflow_supervisor.py` is now the operator-facing workflow contract. It writes `state/workflow_automation.json` every cycle with money truth, scanner/target visibility, risk-governor permission, dispatcher reason, Telegram status, AI support tools, blockers, and next action. The canonical service is `kibot-workflow-supervisor`. Auto-repairable blockers such as `daily_rollover_exit_pending` run the daily reset/executor remediation first; Telegram escalates only if repair persists or fails.

`config/systemd/kibot-daily-reset.service` runs as `ubuntu:ubuntu` so rollover state files remain writable by the supervisor, executor, and dashboard repair loop.
