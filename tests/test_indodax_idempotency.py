...
def test_idempotency_ambiguous_verification_failure_blocks_reentry():
    async def _test():
        ...
        # Pre-trade openOrders returns no orders
        executor.indodax.get_open_orders.side_effect = Exception("Network Error")
        executor.indodax.get_balance.side_effect = [
            100000.0, # IDR balance check for get_balance('idr') (line 1670)
            0.0,      # coin balance before trade (line 2061)
            Exception("Network Error"), # attempt 1 coin check
            Exception("Network Error"), # attempt 2 coin check
            Exception("Network Error"), # attempt 3 coin check
            100000.0, # subsequent call IDR balance check
        ]
        ...
        with patch("Core.Executors.Indodax.indodax_executor.load_live_truth") as mock_truth, \
            ...
            # Process signal under network failures
            await executor.process_signal(signal)
            
            # Verify trade was not opened in active_trades
            assert "SOL/IDR" not in executor.active_trades
            
            # Verify symbol is now placed under verification cooldown
            assert "SOL/IDR" in executor.verification_cooldowns
            assert executor.verification_cooldowns["SOL/IDR"] > 0
            
            # Reset trade mock count to test subsequent process_signal call
            executor.indodax.trade.reset_mock()
            
            # Subsequent signal for SOL/IDR during cooldown period MUST BE BLOCKED
            await executor.process_signal(signal)
            executor.indodax.trade.assert_not_called()
